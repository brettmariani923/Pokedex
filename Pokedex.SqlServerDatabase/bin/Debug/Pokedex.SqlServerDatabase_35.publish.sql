...
BEGIN
    ...
    SELECT 
        [Id], [Name], [HP], [Attack], [Defense], [Speed], [Ability], [Generation], [Region]
 ...
 ) AS v ( [Id], [Name], [HP], [Attack], [Defense], [Speed], [Ability], [Generation], [Region] );
    ...
    MERGE [dbo].[Pokemon] AS p
        USING cte_data AS s ON p.[Id] = s.[Id]
        WHEN NOT MATCHED BY TARGET THEN
            INSERT 
                ([Id], [Name], [HP], [Attack], [Defense], [Speed], [Ability], [Generation], [Region])
            VALUES 
                (s.[Id], s.[Name], s.[HP], s.[Attack], s.[Defense], s.[Speed], s.[Ability], s.[Generation], s.[Region])
        WHEN MATCHED THEN 
            UPDATE SET 
                [Name] = s.[Name], 
                [HP] = s.[HP], 
